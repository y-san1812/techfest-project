...
const columns: { key: Task['status']; label: string; color: string }[] = [
  { key: 'PENDING', label: 'Pending', color: 'border-t-warning' }, // This is fine
                    ...
                    {colTasks.map((task) => (
                      <div key={task.id} className="rounded-lg border border-border bg-background/50 p-3 space-y-2">
                        ...
                        <div className="flex items-center gap-2 pt-1">
                          {col.key !== 'PENDING' && (
                            <button
                              onClick={() => updateStatus(task.id, col.key === 'IN_PROGRESS' ? 'PENDING' : 'IN_PROGRESS')}
                          ...
                          {col.key !== 'COMPLETED' && (
                            <button
                              onClick={() => updateStatus(task.id, col.key === 'PENDING' ? 'IN_PROGRESS' : 'COMPLETED')}
                              className="text-[11px] px-2 py-0.5 rounded border border-border text-muted-foreground hover:text-foreground hover:border-primary/30 transition-colors"
                            >
                              {col.key === 'PENDING' ? 'In Progress' : 'Complete'} →
                            </button>
                          )}
                          <button
