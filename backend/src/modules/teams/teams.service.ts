...
// FIX: Added this function to resolve the 404 error on GET /api/teams
...
// FIX: Corrected progress logic to match your schema (Assignments hold the status)
export async function getTeamProgress(teamId: string) {
  ...
  // Logic: Consider a task "COMPLETED" if at least one assignment is COMPLETED
  const completedTasks = team.tasks.filter(task => 
    task.assignments.some(a => a.status === 'COMPLETED')
  ).length;

  const inProgressTasks = team.tasks.filter(task => 
    !task.assignments.some(a => a.status === 'COMPLETED') && 
    task.assignments.some(a => a.status === 'IN_PROGRESS')
  ).length;
